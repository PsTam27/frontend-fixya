...
const PaymentCard = ( { pago }: { pago: ( typeof MOCK_PAGOS )[0] } ) => {
  ...
  const totalRecibido  = (
    pago.montoCliente - pago.comisionMonto + pago.propina
  )
    ...
    <View style={ styles.card }>
      ...
      <View style={ styles.detailsContainer }>
        <View style={ styles.detailRow }>
          ...
          <Text style={ styles.detailValue }>{ formatCurrency(
            pago.montoCliente ) }</Text>
        </View>
        <View style={ styles.detailRow }>
          <Text style={ styles.detailLabel }>Comisión FixYa
            ({ pago.comisionPorcentaje }%)</Text>
          <Text style={ [
            styles.detailValue, styles.negativeValue
          ] }>-{ formatCurrency( pago.comisionMonto ) }</Text>
        </View>
        <View style={ styles.detailRow }>
          <Text style={ styles.detailLabel }>Propina del cliente</Text>
          <Text style={ [
            styles.detailValue, styles.positiveValue
          ] }>+{ formatCurrency( pago.propina ) }</Text>
        </View>
      </View>
      ...
      <View style={ styles.cardFooter }>
        ...
        <Text style={ styles.paymentMethod }>Pagado
          vía { pago.metodoPago }</Text>
      </View>
    </View>
  )
}
    ...
    <SafeAreaView style={ styles.container }>
      ...
      <ScrollView contentContainerStyle={ styles.scrollContent }>
        ...
        <View style={ styles.balanceCard }>
          ...
          <Text style={ styles.balanceValue }>${ balanceTotal.toLocaleString(
            "es-CL" ) }</Text>
