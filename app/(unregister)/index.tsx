...
export default function RoleSelectionScreen() {
  ...
  const handleSelectRole = ( role: string ) => {
    if ( role === "trabajador" ) {
      router.push( "/(unregister)/(worker-register)/step1" )
    }
    else if ( role === "usuario" ) {
      router.push( "/(unregister)/(user-register)/step1" )
    }
  }
    ...
    <SafeAreaView style={ styles.container }>
      <View style={ styles.mainContent }>
        ...
        <View style={ styles.selectionContainer }>
          <Pressable style={ styles.card }
                     onPress={ () => handleSelectRole( "trabajador" ) }>
            <Image source={ require( "../../assets/images/worker.jpeg" ) }
                   style={ styles.cardImage }/>
            <Text style={ styles.cardText }>Trabajador</Text>
          </Pressable>
          <Pressable style={ styles.card }
                     onPress={ () => handleSelectRole( "usuario" ) }>
      ...
      <View style={ styles.footer }>
        <Text style={ styles.footerText }>
          ...
          <Link href="/(unregister)/login" style={ styles.link } replace>
            Inicia sesión
          </Link>
        </Text>
      </View>
